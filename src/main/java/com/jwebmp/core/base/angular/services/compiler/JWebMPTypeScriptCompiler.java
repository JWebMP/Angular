...
public class JWebMPTypeScriptCompiler {
    /**
     * Utility to run a task with a CallScope, setting the provided source if the scope's source is Unknown/null.
     */
    ...
    public static List<Class<INgApp<?>>> getAllAppsClasses() {
        List<Class<INgApp<?>>> allApps = new ArrayList<>();
        for (ClassInfo classInfo : IGuiceContext
            ...
            try {
                ...
                allApps.add((Class<INgApp<?>>) classInfo.loadClass());
    ...
    public StringBuilder renderAppTS(INgApp<?> appNgApp) throws IOException {
        ...
        try {
            ...
            scan
                    ...
                    .stream()
                    .forEach(a -> {
                        TsDevDependencies annotations = a
                                .loadClass()
            ...
            List<NgAsset> assets = IGuiceContext
                    .get(AnnotationHelper.class)
            ...
            List<NgStyleSheet> ngStyleSheets = IGuiceContext
                    .get(AnnotationHelper.class)
            ...
            List<NgScript> allAnnotations = IGuiceContext
                    .get(AnnotationHelper.class)
            ...
            scan
                    .getClassesWithAnnotation(NgDataService.class)
                    .stream()
                    //.filter(packageFilterClassInfo)
                    .forEach(a -> {
            ...
            scan
                    .getClassesWithAnnotation(NgDataType.class)
                    .stream()
                    //.filter(packageFilterClassInfo)
                    .forEach(a -> {
            ...
            scan
                    .getClassesWithAnnotation(NgServiceProvider.class)
                    .stream()
                    //.filter(packageFilterClassInfo)
                    .forEach(a -> {
    ...
    private void renderAngularApplicationFiles(File currentApp, Class<? extends INgApp<?>> appClass, Map<String, String> namedAssets, INgApp<?> app, ObjectMapper om) throws IOException {
        ...
        try {
            ...
            angularTemplate = angularTemplate.replace("/*BuildStylesSCSS*/",
                    om
                            .writerWithDefaultPrettyPrinter()
                            .writeValueAsString(stylesGlobal));
            angularTemplate = angularTemplate.replace("/*BuildScripts*/",
                    om
                            .writerWithDefaultPrettyPrinter()
                            .writeValueAsString(scripts));
            angularTemplate = angularTemplate.replace("/*MainTSFile*/",
                    om
                            .writerWithDefaultPrettyPrinter()
    ...
    private void processAppConfigFile(File file, ScanResult scan, Class<? extends INgApp<?>> appClass) throws IOException {
        ...
        try (var is = ResourceLocator.class.getResourceAsStream("app.config.json")) {
            ...
            imports.forEach(a -> bootImportsString
                    .append(a)
                    .append("\n"));
            ...
            for (ClassInfo classInfo : scan.getClassesWithAnnotation(NgBootImportProvider.class)) {
                ...
                for (NgBootImportProvider ngBootImportProvider : a) {
                    bootImportProviders
                            .append(ngBootImportProvider.value())
                            .append(",\n");
                }
                //.append(",");
            }
    ...
    private void processPolyfillFile(File file, Class<? extends INgApp<?>> appClass) throws IOException {
        ...
        for (NgPolyfill globalAnnotation : IGuiceContext
                .get(AnnotationHelper.class)
...
*/
    ...
    private boolean processPackageJsonFile(File file, Class<? extends INgApp<?>> appClass, String packageTemplate, ObjectMapper om, Map<String, String> dependencies, Map<String, String> devDependencies, Map<String, String> overrideDependencies, File packageJsonFile) throws IOException {
        ...
        try {
            ...
            packageTemplate = packageTemplate.replace("/*dependencies*/",
                    om
                            .writerWithDefaultPrettyPrinter()
                            .writeValueAsString(dependencies));
            packageTemplate = packageTemplate.replace("/*devDependencies*/",
                    om
                            .writerWithDefaultPrettyPrinter()
                            .writeValueAsString(devDependencies));
            packageTemplate = packageTemplate.replace("/*overrideDependencies*/",
                    om
                            .writerWithDefaultPrettyPrinter()
    ...
    private boolean processStandaloneComponent(File currentApp, NGApplication<?> application, ClassInfo aClass, INgApp<?> app, Class<? extends INgApp<?>> appClass, File finalSrcDirectory) {
        currentAppFile.set(currentApp);
        try {
            //callScoper.enter();
            ...
            var html = ngComponent.toString(0);
            {
                ...
                if (childClass.isAnnotationPresent(NgComponent.class) &&
                    ...
                    StringBuilder cssString = ngComponent
                            .cast()
                            .asStyleBase()
                            .renderCss(1);
    ...
    public StringBuilder renderBootIndexHtml(INgApp<?> app) {
        ...
        body
                .getChildren()
                .clear();
        List<NgComponent> annotations = IGuiceContext
                .get(AnnotationHelper.class)
                .getAnnotationFromClass(app
                        .getAnnotation()
        ...
        if (annotations.isEmpty()) {
            ...
            body.add(new DivSimple<>().setTag(annotations
                    .get(0)
                    .value()));
        }
        p.setBody(body);
            ...
            try {
                ProcessBuilder processBuilder =
                        ...
                        new ProcessBuilder("cmd.exe",
                                "/c",
                                FileUtils.getUserDirectory() + "/AppData/Roaming/npm/npm.cmd",
                                "run",
                                "build" + (IGuiceContext
                                        .get(EnvironmentModule.class)
                                        .getEnvironmentOptions()
