...
public class ConfigureImportReferences implements IOnComponentConfigured<ConfigureImportReferences> {
    ...
    private boolean isStandaloneComponent(IComponentHierarchyBase<?, ?> component) {
        return INgComponent.class.isAssignableFrom(component.getClass()) && component
                .getClass()
    ...
    private void onComponentConfigured(IComponentHierarchyBase<GlobalChildren, ?> parent, IComponentHierarchyBase<GlobalChildren, ?> component, boolean checkForParent) {
        ...
        for (GlobalChildren child : disconnectedChildren) {
            ...
            if (!checkForParent) {
                if (INgComponent.class.isAssignableFrom(child.getClass())
                        && child
                        .getClass()
                        .getDeclaredAnnotationsByType(NgComponent.class).length > 0) {
                    compConfig
                            .getImportReferences()
                    ...
                    if (compConfig.getRootComponent() instanceof ImportsStatementsComponent<?> imp) {
                        ...
                        compConfig
                                .getImportModules()
                                .add(AnnotationUtils.getNgImportModule(ngImportReferences
                                        .getFirst()
                                        .value()));
                    ...
                    ((IComponentHierarchyBase) compConfig.getRootComponent())
                            .getConfigurations()
        ...
        if (parent == null) {
            ...
            component
                    .asBase()
                    .getProperties()
                    .put("AngularConfiguration", compConfig)
            ;
        }
    }
    ...
    private void updateTag(List<GlobalChildren> childList, IComponentHierarchyBase<?, ?> component) {
        ...
        var anno = component
                .getClass()
                .getAnnotationsByType(NgComponent.class)[0];
        ...
        for (NgOutput a : outputs) {
            if (uniqueOutValues.add(a)) {
                if (!Strings.isNullOrEmpty(a.parentMethodName())) {
                    replacementTag
                            .asAttributeBase()
                            .getAttributes()
        ...
        for (IConfiguration configuration : component.getConfigurations()) {
            if (configuration instanceof NgComponentReference ngComponentReference && component instanceof ImportsStatementsComponent<?> imp && compConfig instanceof INgComponent) {
                ...
                if (INgDirective.class.isAssignableFrom(configReferenceClass) ||
                    ...
                    if (!ngComponentReference.referenceOnly()) {
                        compConfig
                                .getImportModules()
                                .add(AnnotationUtils.getNgImportModule(ngImportReferences
                                        .getFirst()
                ...
                if (ngComponentReference.onSelf() && !checkForParent) {
                    compConfig
                            .getImportReferences()
                            .add(AnnotationUtils.getNgImportReference(ngComponentReference.value(), ngComponentReference.reference()));
                }
                ...
                if (ngComponentReference.onSelf() && !checkForParent) {
                    compConfig
                            .getInterfaces()
                            .add(AnnotationUtils.getNgInterface(ngComponentReference.value()));
                }
                if (ngComponentReference.onParent() && checkForParent) {
                    compConfig
                            .getInterfaces()
                ...
                if ((ngComponentReference.onSelf() && !checkForParent) || (ngComponentReference.onParent() && checkForParent)) {
                    ...
                    compConfig
                            .getOnInit()
                            .add(AnnotationUtils.getNgOnInit(ngComponentReference.value()));
                }
                ...
                if ((ngComponentReference.onSelf() && !checkForParent) || (ngComponentReference.onParent() && checkForParent)) {
                    ...
                    compConfig
                            .getOnDestroy()
                            .add(AnnotationUtils.getNgOnDestroy(ngComponentReference.value()));
                }
            } else if (configuration instanceof NgAfterViewInit ngComponentReference) {
                if ((ngComponentReference.onSelf() && !checkForParent) || (ngComponentReference.onParent() && checkForParent)) {
                    compConfig
                            .getImportReferences()
                            .add(AnnotationUtils.getNgImportReference("AfterViewInit", "@angular/core"));
                    compConfig
                            .getInterfaces()
                            .add(AnnotationUtils.getNgInterface("AfterViewInit"));
                    compConfig
                            .getAfterViewInit()
                            .add(AnnotationUtils.getNgAfterViewInit(ngComponentReference.value()));
                }
            } else if (configuration instanceof NgAfterViewChecked ngComponentReference) {
                if ((ngComponentReference.onSelf() && !checkForParent) || (ngComponentReference.onParent() && checkForParent)) {
                    compConfig
                            .getImportReferences()
                            .add(AnnotationUtils.getNgImportReference("AfterViewChecked", "@angular/core"));
                    compConfig
                            .getInterfaces()
                            .add(AnnotationUtils.getNgInterface("AfterViewChecked"));
                    compConfig
                            .getAfterViewChecked()
                            .add(AnnotationUtils.getNgAfterViewChecked(ngComponentReference.value()));
                }
            } else if (configuration instanceof NgAfterContentInit ngComponentReference) {
                if ((ngComponentReference.onSelf() && !checkForParent) || (ngComponentReference.onParent() && checkForParent)) {
                    compConfig
                            .getImportReferences()
                            .add(AnnotationUtils.getNgImportReference("AfterContentInit", "@angular/core"));
                    compConfig
                            .getInterfaces()
                            .add(AnnotationUtils.getNgInterface("AfterContentInit"));
                    compConfig
                            .getAfterContentInit()
                            .add(AnnotationUtils.getNgAfterContentInit(ngComponentReference.value()));
                }
            } else if (configuration instanceof NgAfterContentChecked ngComponentReference) {
                if ((ngComponentReference.onSelf() && !checkForParent) || (ngComponentReference.onParent() && checkForParent)) {
                    compConfig
                            .getImportReferences()
                            .add(AnnotationUtils.getNgImportReference("AfterContentChecked", "@angular/core"));
                    compConfig
                            .getInterfaces()
                            .add(AnnotationUtils.getNgInterface("AfterContentChecked"));
                    compConfig
                            .getAfterContentChecked()
                            .add(AnnotationUtils.getNgAfterContentChecked(ngComponentReference.value()));
                }
                ...
                if (ngComponentReference.onSelf() && !checkForParent) {
                    compConfig
                            .getImportModules()
                            .add(AnnotationUtils.getNgImportModule(ngComponentReference.value()));
                }
                if (ngComponentReference.onParent() && checkForParent) {
                    compConfig
                            .getImportModules()
                ...
                if (ngComponentReference.onSelf() && !checkForParent) {
                    compConfig
                            .getImportProviders()
                            .add(AnnotationUtils.getNgImportProvider(ngComponentReference.value()));
                }
                if (ngComponentReference.onParent() && checkForParent) {
                    compConfig
                            .getImportProviders()
                ...
                if (ngComponentReference.onSelf() && !checkForParent) {
                    compConfig
                            .getGlobalFields()
                            .add(AnnotationUtils.getNgGlobalField(ngComponentReference.value()));
                }
                if (ngComponentReference.onParent() && checkForParent) {
                    compConfig
                            .getGlobalFields()
                ...
                if (ngComponentReference.onSelf() && !checkForParent) {
                    compConfig
                            .getImportReferences()
                            .add(AnnotationUtils.getNgImportReference("signal", "@angular/core"));
                    compConfig
                            .getImportReferences()
                            .add(AnnotationUtils.getNgImportReference("computed", "@angular/core"));
                    compConfig
                            .getSignals()
                            .add(AnnotationUtils.getNgSignal(ngComponentReference.referenceName(), ngComponentReference.value(), ngComponentReference.type()));
                }
                if (ngComponentReference.onParent() && checkForParent) {
                    compConfig
                            .getImportReferences()
        ...
        if (comp instanceof INgComponent<?> component) {
            for (String afterContentChecked : component.afterContentChecked()) {
                if (!Strings.isNullOrEmpty(afterContentChecked)) {
                    ...
                    compConfig
                            .getAfterContentChecked()
                            .add(AnnotationUtils.getNgAfterContentChecked(afterContentChecked));
                }
            }
            for (String afterContentInit : component.afterContentInit()) {
                if (!Strings.isNullOrEmpty(afterContentInit)) {
                    ...
                    compConfig
                            .getAfterContentInit()
                            .add(AnnotationUtils.getNgAfterContentInit(afterContentInit));
                }
            }
            ...
            for (String afterViewInit : component.afterViewInit()) {
                if (!Strings.isNullOrEmpty(afterViewInit)) {
                    ...
                    compConfig
                            .getAfterViewInit()
                            .add(AnnotationUtils.getNgAfterViewInit(afterViewInit));
                }
            }
            for (String afterViewChecked : component.afterViewChecked()) {
                if (!Strings.isNullOrEmpty(afterViewChecked)) {
                    ...
                    compConfig
                            .getAfterViewChecked()
                            .add(AnnotationUtils.getNgAfterViewChecked(afterViewChecked));
                }
            }
        }
        ...
        if (comp instanceof INgComponent<?> component) {
            for (String provider : component.providers()) {
                if (!Strings.isNullOrEmpty(provider)) {
                    ...
                    if ((ng.isOnSelf() && !checkForParent) || (ng.isOnParent() && checkForParent)) {
                        compConfig
                                .getImportProviders()
                                .add(AnnotationUtils.getNgImportProvider(ng.value()));
                    }
                }
            }
            for (String input : component.inputs()) {
                if (!Strings.isNullOrEmpty(input)) {
                    ...
                    compConfig
                            .getInputs()
                            .add(AnnotationUtils.getNgInput(ng.value()));
                }
            }
        }
    }
    ...
    private void addFormsImports(IComponentHierarchyBase<GlobalChildren, ?> component, boolean checkForParent) {
        if (component
                .asAttributeBase()
                .getAttributes()
                .containsValue("ngModel") || component
                .asAttributeBase()
                .getAttributes()
        ...
        if (component
            ...
            compConfig
                    .getImportReferences()
                    .add(AnnotationUtils.getNgImportReference("NgClass", "@angular/common"));
            compConfig
                    .getImportModules()
                    .add(AnnotationUtils.getNgImportModule("NgClass"));
        }
        if (component
                .asAttributeBase()
                .getAttributes()
            ...
            compConfig
                    .getImportReferences()
                    .add(AnnotationUtils.getNgImportReference("NgStyle", "@angular/common"));
            compConfig
                    .getImportModules()
                    .add(AnnotationUtils.getNgImportModule("NgStyle"));
        }
    ...
    private void addConstructorParameters(Class<?> component, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(component, NgConstructorParameter.class)
    ...
    private void addConstructorBodies(Class<?> component, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(component, NgConstructorBody.class)
    ...
    private void addFields(Class<?> component, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(component, NgField.class)
                .forEach(ngField -> {
    ...
    private void addGlobalFields(Class<?> component, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(component, NgGlobalField.class)
                .forEach(ngField -> {
                    if ((ngField.onSelf() && !checkForParent) || (ngField.onParent() && checkForParent)) {
                        compConfig
                                .getGlobalFields()
    ...
    private void addMethods(Class<?> component, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(component, NgMethod.class)
                .forEach(ngMethod -> {
    ...
    private void addInterfaces(Class<?> component, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(component, NgInterface.class)
                .forEach(ngMethod -> {
                    if ((ngMethod.onSelf() && !checkForParent) || (ngMethod.onParent() && checkForParent)) {
                        compConfig
                                .getInterfaces()
    ...
    private void addInjects(Class<?> component, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(component, NgInject.class)
                .forEach(ngMethod -> {
                    if ((ngMethod.onSelf() && !checkForParent) || (ngMethod.onParent() && checkForParent)) {
                        compConfig
                                .getInjects()
    ...
    private void addImportModules(Class<?> component, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(component, NgImportModule.class)
                .forEach(ngMethod -> {
                    if ((ngMethod.onSelf() && !checkForParent) || (ngMethod.onParent() && checkForParent)) {
                        compConfig
                                .getImportModules()
                                .add(AnnotationUtils.getNgImportModule(ngMethod.value()));
                    }
                });
    }
    ...
    private void addImportProviders(Class<?> component, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(component, NgImportProvider.class)
                .forEach(ngMethod -> {
                    if ((ngMethod.onSelf() && !checkForParent) || (ngMethod.onParent() && checkForParent)) {
                        compConfig
                                .getImportProviders()
                                .add(AnnotationUtils.getNgImportProvider(ngMethod.value()));
                    }
                });
    }
    ...
    private void addModels(Class<?> component, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(component, NgModel.class)
                .forEach(ngMethod -> {
                    if ((ngMethod.onSelf() && !checkForParent) || (ngMethod.onParent() && checkForParent)) {
                        compConfig
                                .getModels()
    ...
    private void addSignals(Class<?> component, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(component, NgSignal.class)
                .forEach(ngMethod -> {
                    if ((ngMethod.onSelf() && !checkForParent) || (ngMethod.onParent() && checkForParent)) {
                        compConfig
                                .getImportReferences()
                        ...
                        compConfig
                                .getSignals()
                                .add(AnnotationUtils.getNgSignal(ngMethod.referenceName(), ngMethod.value(), ngMethod.type()));
                    }
                });
    }

    private void addSignalComputeds(Class<?> component, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(component, NgSignalComputed.class)
                .forEach(ngMethod -> {
                    if ((ngMethod.onSelf() && !checkForParent) || (ngMethod.onParent() && checkForParent)) {
                        compConfig
                                .getImportReferences()
                        ...
                        compConfig
                                .getSignalComputeds()
                                .add(AnnotationUtils.getNgSignalComputed(ngMethod.referenceName(), ngMethod.value()));
                    }
                });
    }

    private void addSignalEffects(Class<?> component, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(component, NgSignalEffect.class)
                .forEach(ngMethod -> {
                    if ((ngMethod.onSelf() && !checkForParent) || (ngMethod.onParent() && checkForParent)) {
                        compConfig
                                .getImportReferences()
                                .add(AnnotationUtils.getNgImportReference("effect", "@angular/core"));
                        compConfig
                                .getSignalEffects()
                                .add(AnnotationUtils.getNgSignalEffect(ngMethod.referenceName(), ngMethod.value()));
                    }
                });
    }
    ...
    private void addInputs(Class<?> component, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(component, NgInput.class)
                .forEach(ngMethod -> {
                    if ((ngMethod.onSelf() && !checkForParent) || (ngMethod.onParent() && checkForParent)) {
                        compConfig
                                .getInputs()
                                .add(AnnotationUtils.getNgInput(
                                        ngMethod.value(),
                                        ngMethod.mandatory(),
                                        ngMethod.type(),
    ...
    private void addOutputs(Class<?> component, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(component, NgOutput.class)
                .forEach(ngMethod -> {
                    if ((ngMethod.onSelf() && !checkForParent) || (ngMethod.onParent() && checkForParent)) {
                        compConfig
                                .getImportReferences()
    ...
    private void addOnInit(Class<?> componentClass, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(componentClass, NgOnInit.class)
                .forEach(ngMethod -> {
                    if ((ngMethod.onSelf() && !checkForParent) || (ngMethod.onParent() && checkForParent)) {
                        compConfig
                                .getImportReferences()
                        ...
                        compConfig
                                .getOnInit()
                                .add(AnnotationUtils.getNgOnInit(ngMethod
                                        .value()
                                        .trim()));
                    }
                });
    ...
    private void addOnDestroy(Class<?> componentClass, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(componentClass, NgOnDestroy.class)
                .forEach(ngMethod -> {
                    if ((ngMethod.onSelf() && !checkForParent) || (ngMethod.onParent() && checkForParent)) {
                        compConfig
                                .getImportReferences()
    ...
    private void addAfterViewInit(Class<?> componentClass, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(componentClass, NgAfterViewInit.class)
                .forEach(ngMethod -> {
                    if ((ngMethod.onSelf() && !checkForParent) || (ngMethod.onParent() && checkForParent)) {
                        compConfig
                                .getImportReferences()
                        ...
                        compConfig
                                .getAfterViewInit()
                                .add(AnnotationUtils.getNgAfterViewInit(ngMethod.value()));
                    }
                });
    }

    private void addAfterViewChecked(Class<?> componentClass, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(componentClass, NgAfterViewChecked.class)
                .forEach(ngMethod -> {
                    if ((ngMethod.onSelf() && !checkForParent) || (ngMethod.onParent() && checkForParent)) {
                        compConfig
                                .getImportReferences()
                        ...
                        compConfig
                                .getAfterViewChecked()
                                .add(AnnotationUtils.getNgAfterViewChecked(ngMethod.value()));
                    }
                });
    }

    private void addAfterContentChecked(Class<?> componentClass, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(componentClass, NgAfterContentChecked.class)
                .forEach(ngMethod -> {
                    if ((ngMethod.onSelf() && !checkForParent) || (ngMethod.onParent() && checkForParent)) {
                        compConfig
                                .getImportReferences()
                        ...
                        compConfig
                                .getAfterContentChecked()
                                .add(AnnotationUtils.getNgAfterContentChecked(ngMethod.value()));
                    }
                });
    }

    private void addAfterContentInit(Class<?> componentClass, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(componentClass, NgAfterContentInit.class)
                .forEach(ngMethod -> {
                    if ((ngMethod.onSelf() && !checkForParent) || (ngMethod.onParent() && checkForParent)) {
                        compConfig
                                .getImportReferences()
                        ...
                        compConfig
                                .getAfterContentInit()
                                .add(AnnotationUtils.getNgAfterContentInit(ngMethod.value()));
                    }
                });
    }


    private void addImportReferences(Class<?> componentClass, boolean checkForParent) {
        AnnotationUtils
                .getAnnotation(componentClass, NgImportReference.class)
                ...
                .forEach(importReference -> {
                    if ((importReference.onSelf() && !checkForParent) || (importReference.onParent() && checkForParent)) {
                        ...
                        if (
                            ...
                            compConfig
                                    .getImportModules()
                                    .add(AnnotationUtils.getNgImportModule(AnnotationUtils.getTsFilename(importReference.value())));
                        ...
                        // For INgDataType references (not covered above), also process onParent annotations
                        if (INgDataType.class.isAssignableFrom(importReference.value())
    ...
    private List<NgImportReference> retrieveRelativePathForReference(NgComponentReference importReference) {
        ...
        }.putRelativeLinkInMap(compConfig
                .getRootComponent()
                .getClass(), importReference);
        return irs;
    }
    ...
    private void processClassToComponent(Class<?> componentClass, IComponentHierarchyBase<GlobalChildren, ?> component, boolean checkForParent) {
        ...
        addAfterContentChecked(componentClass, checkForParent);
        addAfterContentInit(componentClass, checkForParent);
        addAfterViewChecked(componentClass, checkForParent);
        addAfterViewInit(componentClass, checkForParent);
        ...
        addSignalComputeds(componentClass, checkForParent);
        addSignalEffects(componentClass, checkForParent);
        ...
        for (var reference : references) {
            ...
            if (configClass.isAnnotationPresent(NgComponent.class)) {
                ...
                if (!anno.singleton()) {
                    compConfig
                            .getImportProviders()
                            .add(AnnotationUtils.getNgImportProvider(AnnotationUtils.getTsFilename(configClass)));
                }
                compConfig
                        .getInjects()
        ...
        for (NgOnInit ni : AnnotationUtils.getAnnotation(componentClass, NgOnInit.class)) {
            if (!Strings.isNullOrEmpty(ni.value())) {
                if ((ni.onSelf() && !checkForParent) || (ni.onParent() && checkForParent)) {
                    compConfig
                            .getOnInit()
                            .add(AnnotationUtils.getNgOnInit(ni.value()));
                    compConfig
                            .getImportReferences()
                            .add(AnnotationUtils.getNgImportReference("OnInit", "@angular/core"));
                    compConfig
        ...
        for (NgOnDestroy ni : AnnotationUtils.getAnnotation(componentClass, NgOnDestroy.class)) {
            if (!Strings.isNullOrEmpty(ni.value())) {
                if ((ni.onSelf() && !checkForParent) || (ni.onParent() && checkForParent)) {
                    compConfig
                            .getOnDestroy()
                            .add(AnnotationUtils.getNgOnDestroy(ni.value()));
                    compConfig
                            .getImportReferences()
                            .add(AnnotationUtils.getNgImportReference("OnDestroy", "@angular/core"));
                    compConfig
        ...
        for (NgAfterContentChecked ni : AnnotationUtils.getAnnotation(componentClass, NgAfterContentChecked.class)) {
            if (!Strings.isNullOrEmpty(ni.value())) {
                if ((ni.onSelf() && !checkForParent) || (ni.onParent() && checkForParent)) {
                    compConfig
                            .getAfterContentChecked()
                            .add(AnnotationUtils.getNgAfterContentChecked(ni.value()));
                    compConfig
                            .getImportReferences()
                            .add(AnnotationUtils.getNgImportReference("AfterContentChecked", "@angular/core"));
                    compConfig
        ...
        for (NgAfterContentInit ni : AnnotationUtils.getAnnotation(componentClass, NgAfterContentInit.class)) {
            if (!Strings.isNullOrEmpty(ni.value())) {
                if ((ni.onSelf() && !checkForParent) || (ni.onParent() && checkForParent)) {
                    compConfig
                            .getAfterContentInit()
                            .add(AnnotationUtils.getNgAfterContentInit(ni.value()));
                    compConfig
                            .getImportReferences()
                            .add(AnnotationUtils.getNgImportReference("AfterContentInit", "@angular/core"));
                    compConfig
        ...
        for (NgAfterViewChecked ni : AnnotationUtils.getAnnotation(componentClass, NgAfterViewChecked.class)) {
            if (!Strings.isNullOrEmpty(ni.value())) {
                if ((ni.onSelf() && !checkForParent) || (ni.onParent() && checkForParent)) {
                    compConfig
                            .getAfterViewChecked()
                            .add(AnnotationUtils.getNgAfterViewChecked(ni.value()));
                    compConfig
                            .getImportReferences()
                            .add(AnnotationUtils.getNgImportReference("AfterViewChecked", "@angular/core"));
                    compConfig
        ...
        for (NgAfterViewInit ni : AnnotationUtils.getAnnotation(componentClass, NgAfterViewInit.class)) {
            if (!Strings.isNullOrEmpty(ni.value())) {
                if ((ni.onSelf() && !checkForParent) || (ni.onParent() && checkForParent)) {
                    compConfig
                            .getAfterViewInit()
                            .add(AnnotationUtils.getNgAfterViewInit(ni.value()));
                    compConfig
                            .getImportReferences()
                            .add(AnnotationUtils.getNgImportReference("AfterViewInit", "@angular/core"));
                    compConfig
        ...
        if (component instanceof INgComponent<?> ngComponent && !checkForParent) {
            for (String s : ngComponent.onInit()) {
                if (!Strings.isNullOrEmpty(s)) {
                    compConfig
                            .getOnInit()
                            .add(AnnotationUtils.getNgOnInit(s.trim()));
                    compConfig
                            .getInterfaces()
                            .add(AnnotationUtils.getNgInterface("OnInit"));
                    compConfig
            ...
            for (String s : ngComponent.onDestroy()) {
                if (!Strings.isNullOrEmpty(s)) {
                    compConfig
                            .getOnDestroy()
                            .add(AnnotationUtils.getNgOnDestroy(s.trim()));
                    compConfig
                            .getInterfaces()
                            .add(AnnotationUtils.getNgInterface("OnDestroy"));
                    compConfig
            ...
            for (String s : ngComponent.afterViewInit()) {
                if (!Strings.isNullOrEmpty(s)) {
                    compConfig
                            .getAfterViewInit()
                            .add(AnnotationUtils.getNgAfterViewInit(s.trim()));
                    compConfig
                            .getInterfaces()
                            .add(AnnotationUtils.getNgInterface("AfterViewInit"));
                    compConfig
            ...
            for (String s : ngComponent.afterViewChecked()) {
                if (!Strings.isNullOrEmpty(s)) {
                    compConfig
                            .getAfterViewChecked()
                            .add(AnnotationUtils.getNgAfterViewChecked(s.trim()));
                    compConfig
                            .getInterfaces()
                            .add(AnnotationUtils.getNgInterface("AfterViewChecked"));
                    compConfig
            ...
            for (String s : ngComponent.afterContentInit()) {
                if (!Strings.isNullOrEmpty(s)) {
                    compConfig
                            .getAfterContentInit()
                            .add(AnnotationUtils.getNgAfterContentInit(s.trim()));
                    compConfig
                            .getInterfaces()
                            .add(AnnotationUtils.getNgInterface("AfterContentInit"));
                    compConfig
            ...
            for (String s : ngComponent.afterContentChecked()) {
                if (!Strings.isNullOrEmpty(s)) {
                    compConfig
                            .getAfterContentChecked()
                            .add(AnnotationUtils.getNgAfterContentChecked(s.trim()));
                    compConfig
                            .getInterfaces()
                            .add(AnnotationUtils.getNgInterface("AfterContentChecked"));
                    compConfig
    ...
    private void addPipes(IComponentHierarchyBase<?, ?> component, boolean checkForParent) {
        boolean formsAdded = false;
        for (String key : component
                .asAttributeBase()
                .getAttributes()
                .keySet()) {
            if (!formsAdded && !Strings.isNullOrEmpty(key) && key.contains("[(ngModel)]")) {
                compConfig
                        .getImportReferences()
                        .add(AnnotationUtils.getNgImportReference("FormsModule", "@angular/forms"));
                compConfig
        ...
        // Also detect ngModel usage in template text
        if (!formsAdded) {
            if (!Strings.isNullOrEmpty(componentString) && componentString.contains("[(ngModel)]")) {
                compConfig
                        .getImportReferences()
                        .add(AnnotationUtils.getNgImportReference("FormsModule", "@angular/forms"));
                compConfig
        ...
        // Pre-check by text content for other common pipes
        if (componentString.contains("| number")) {
            compConfig
                    .getImportReferences()
        ...
        if (componentString.contains("| async")) {
            compConfig
                    .getImportReferences()
        ...
        if (componentString.contains("| date")) {
            compConfig
                    .getImportReferences()
                    .add(AnnotationUtils.getNgImportReference("DatePipe", "@angular/common"));
            compConfig
                    .getImportModules()
        ...
        for (String value : component
                ...
                .getAttributes()
                .values()) {
            if (!jsonAdded && !Strings.isNullOrEmpty(value) && value.contains("| json")) {
                compConfig
                        .getImportReferences()
                        .add(AnnotationUtils.getNgImportReference("JsonPipe", "@angular/common"));
                compConfig
            ...
            if (!decimalAdded && !Strings.isNullOrEmpty(value) && value.contains("| number")) {
                compConfig
                        .getImportReferences()
                        .add(AnnotationUtils.getNgImportReference("DecimalPipe", "@angular/common"));
                compConfig
            ...
            if (!asyncAdded && !Strings.isNullOrEmpty(value) && value.contains("| async")) {
                compConfig
                        .getImportReferences()
                        .add(AnnotationUtils.getNgImportReference("AsyncPipe", "@angular/common"));
                compConfig
        ...
        if (!jsonAdded && componentString.contains("| json")) {
            compConfig
                    .getImportReferences()
                    .add(AnnotationUtils.getNgImportReference("JsonPipe", "@angular/common"));
            compConfig
        ...
        if (!asyncAdded && componentString.contains("| async")) {
            compConfig
                    .getImportReferences()
                    .add(AnnotationUtils.getNgImportReference("AsyncPipe", "@angular/common"));
            compConfig
        ...
        for (String value : component
                ...
                .getAttributes()
                .values()) {
            if (!dateAdded && !Strings.isNullOrEmpty(value) && value.contains("| date")) {
                compConfig
                        .getImportReferences()
                        .add(AnnotationUtils.getNgImportReference("DatePipe", "@angular/common"));
                compConfig
        ...
        if (!ngInputs.isEmpty()) {
            for (NgInput a : ngInputs) {
                if ((a.onSelf() && !checkForParent) || (a.onParent() && checkForParent)) {
                    ...
                    if (a.type() != null && !a
                            .type()
                            .isAnnotationPresent(NgIgnoreImportReference.class)) {
                        ...
                        for (NgImportReference inputReference : inputReferences) {
                            compConfig
                                    .getImportReferences()
                                    .add(AnnotationUtils.getNgImportReference(inputReference.value(), inputReference.reference()));
                        }
                    }
                }
            }
